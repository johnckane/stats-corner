...
trans <- read.csv("/home/john/stats_corner/Datasets/transactions_history.csv",header=TRUE)
...
data_summary %>% arrange(desc(sum_pw))
...
plot1a <- ggplot(data=subset(data2,trades <= 5),aes(x=trades,y=sum_pw,label=label))
plot1a+
  geom_point() +
  scale_x_continuous("# of Trades Made",
                     limits=c(0,11)) +
...
plot1b <- ggplot(data=data2,aes(x=trades,y=sum_pw,group=year))
plot1b+
 geom_point() +
  scale_x_continuous("# of Trades Made",
                     limits=c(-1,11)) +
  scale_y_continuous("Proportional Wins") +
  geom_dl(aes(label=owner),list(cex=0.65)) +
  theme(legend.position = 'none') +
  ggtitle("Proportional Wins by Trades Made") +
  geom_smooth(method=lm,se=FALSE) +
