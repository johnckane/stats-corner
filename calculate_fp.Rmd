...
filter(points_week_data_w_cost2, player == 'TB-2300')
...
player_year <- drafted_points_w_week %>% group_by(player,year) %>% slice(1) %>% mutate(dummy = 1) %>% select(player,year,dummy)
...
points_week_data_w_cost3 <-
  player_year_week %>%
  select(-dummy) %>%
  left_join(.,points_week_data_w_cost2, by = c("player","year","week")) %>%
  filter(week %in% c(1:16,30,40)) %>%
...
transpose2010 <- points_week_data_w_cost3 %>%
  ...
  mutate(temp2 = str_replace_all(temp2,c("/"),"")) %>%
  mutate(temp2 = str_replace_all(temp2,c("-"),"")) %>%
  mutate(temp2 = str_replace_all(temp2,c("49ers"),".49ers")) %>%  spread(key = temp2, value = points) %>%
  select_(.,.dots = names2010)
  
names2010[1]

select_(transpose2010)  
parse(text = (names2010)[[1]])
