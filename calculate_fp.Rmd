...
Merge draft cost with stats. Need an observation with week -1 and 0 for keeper and value data.
```{r}
head(points_week_data)

new_rows_df <- data.frame(week = c(-1,0))
...
points_week_data_w_cost <-
  ...
  mutate(points = ifelse(week == 0,value,
                         ifelse(week == -1, keeper,points)))
