...
f <- function(p,sp){
    df <- adp_draft %>% filter(pos == p)
    return(loess(data=df, value~pick, span = sp))
}
## QB ##
qb_25  <- f("QB", 0.25)
qb_50  <- f("QB", 0.50)
...
colnames(qb) <- c("pick","fitted_cost")
colnames(rb) <- c("pick","fitted_cost")
colnames(wr) <- c("pick","fitted_cost")
colnames(te) <- c("pick","fitted_cost")
...
fitted_cost <- sqldf('
                    ...
                    order by
                        pos, pick
                    ')
write.csv(fitted_cost,"loess_cost.csv")
